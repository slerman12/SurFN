...
class StochasticPolicyGradientSurFN(updaters.StochasticPolicyGradient):
    ...
    def __call__(self, observations, actions, advantages, log_probs):
        if (advantages == 0.).all():
            ...
            surfn_sweep.set_fittest(self.model.actor, new_log_probs, advantages)
            self.optimizer.zero_grad()
    ...
    def __call__(self, observations, actions, advantages, log_probs):
        if (advantages == 0.).all():
            ...
            if self.resample or self.probas is None:
                # todo call even if not resample! but don't sample in that case (add hook?)
                probas = surfn_sweep.set_fittest(policy=self.model.actor, advantages=advantages, probas=self.probas,
                                                 log_probs=log_probs, selection_rate=self.selection_rate,
                                                 min_adv=self.min_adv, **self.kwargs)

                if not self.repeat:
                    self.probas = probas

            elif not self.reiterate:
                surfn_sweep.reset(self.model.actor)
