...
class StochasticPolicyGradientSurFN(updaters.StochasticPolicyGradient):
    ...
    def __call__(self, observations, actions, advantages, log_probs):
        if (advantages == 0.).all():
            ...
            surfn_2.set_fittest(self.model.actor, new_log_probs, advantages)
            self.optimizer.zero_grad()
...
class ClippedRatioSurFN(updaters.ClippedRatio):
    ...
    def __call__(self, observations, actions, advantages, log_probs):
        if (advantages == 0.).all():
            ...
            policy_loss = -(torch.min(surrogates_1, surrogates_2)).mean()

            # clamped = torch.is_nonzero(ratios_2)
            # new_log_probs = new_log_probs * clamped
            ...
            if self.resample or self.probas is None:
                probas = surfn_2.set_fittest(self.model.actor, policy_loss, advantages, self.optimizer, self.probas,
                                             selection_rate=self.selection_rate, min_adv=self.min_adv,
                                             log_probs=log_probs,  # todo wo
                                             **self.kwargs)
                # policy_loss.backward()
                # og = torch.cat([param.grad.flatten() for param in self.model.actor.parameters()], dim=0)
                # autograd_hacks.compute_grad1(self.model.actor)
                #
                # adv_grads = torch.cat([param.grad1.flatten(start_dim=1) for param in self.model.actor.parameters()
                #                        if hasattr(param, 'grad1')], dim=1)
                #
                # actual = torch.cat([param.grad.flatten() for param in self.model.actor.parameters() if hasattr(param, 'grad1')], dim=0)
                # ng = torch.cat([param.grad.flatten() for param in self.model.actor.parameters()], dim=0)
                # assert torch.eq(ng, og).all()
                #
                # if not torch.allclose(torch.mean(adv_grads, dim=0), actual):
                ...
                #         new_grads = surfn_2.get_survival_hook(None, False)(new_grads)
                #         assert torch.allclose(param.grad, new_grads)
                ...
                surfn_2.reset(self.model.actor)
            else:
                surfn_2.set_fittest(self.model.actor, policy_loss, advantages, self.optimizer, self.probas,
                                    do_surfn=False)
            ...
            loss = (policy_loss + entropy_loss).detach()
            with torch.no_grad():
