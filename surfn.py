...
def get_fittest(policy, log_probs, advantages, selection_rate=0.9, seed=None):
    ...
    for param in policy.parameters():
        if hasattr(param, 'grad1'):
            param_fittest = fittest[fitness_size:fitness_size + np.prod(param.shape)]
...
#     """
#
#     # if hasattr(policy, 'surfn_hook'):
#     #     policy.surfn_hook.remove()
#     #     del policy.surfn_hook
#
#     log_probs.sum().backward(retain_graph=True)
#     autograd_hacks.compute_grad1(policy)
#
#     # todo abs val instead of relu and softmax * advantages for fitness
#     grads = torch.relu(torch.cat([torch.cat([param.grad1.flatten(start_dim=1) for param in layer.parameters()], dim=1)
#                         for layer in policy.modules() if autograd_hacks.is_supported(layer)], dim=1))
#
#     grads_sum = grads.sum(dim=0)
#     grads_advantages_sum = torch.sum(grads * advantages[:, None], dim=0)
#
#     fitness = grads_advantages_sum / grads_sum
#     # todo compatibility with negative rewards?
#     fitness[torch.isnan(fitness)] = 0
...
#     # params = torch.cat([torch.cat([param.flatten() for param in layer.parameters()])
#     #                     for layer in policy.modules() if autograd_hacks.is_supported(layer)])
#     # policy.__dict__['surfn_hook'] = params.register_hook(get_hook(fittest))
#     return fittest


def save_fittest(policy, fittest):
    """
    Nullify/freeze gradients for the fittest policy parameters.
